<script setup>
import { getMonth, getDaysInMonth, startOfWeek, getWeeksInMonth, getDay, startOfMonth, endOfMonth, addDays, lightFormat } from 'date-fns'
...
<template>
  <main>
    ...
    <article v-for="week in monthModel" :key="week" class="grid grid-cols-7 text-14 justify-items-center">
      <p class="mb-0" v-for="d in week" :key="d">
        <span v-if="getMonth(d) === month">{{ lightFormat(d, 'dd') }}</span>
      </p>
    </article>
    </section>
  </main>
